package cn.bdqn.urltest.common.util;
...
public class RedisUtil {
     ...
     * @return
     */
    public boolean addZset(String key,List<?> list,long time){
        if(list != null){
            for (int i=0;i<list.size();i++){
                redisTemplate.opsForZSet().add(key, JSON.toJSONString(list.get(i)),i+1);
                redisTemplate.expire(key,time,TimeUnit.SECONDS);
            }
        }else{
